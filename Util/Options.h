...
public:
    ...
    Next(std::optional<T> *result, std::vector<std::string> const &args, std::vector<std::string>::const_iterator *it, bool allowDuplicate = false);
...
public:
    ...
    template<typename T>
    static std::pair<bool, std::string>
    Current(std::optional<T> *result, std::string const &arg, bool allowDuplicate = false);
