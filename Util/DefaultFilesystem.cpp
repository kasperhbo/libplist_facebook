...
std::optional<Filesystem::Type> DefaultFilesystem::
...
std::optional<Permissions> DefaultFilesystem::
...
writeFilePermissions(std::string const &path, Permissions::Operation operation, Permissions permissions)
{
    ...
    return true;
#else
    std::optional<Permissions> updated = this->readFilePermissions(path);
    if (!updated) {
        return false;
    }
...
writeSymbolicLinkPermissions(std::string const &path, Permissions::Operation operation, Permissions permissions)
{
    ...
    std::optional<Permissions> updated = this->readSymbolicLinkPermissions(path);
    if (!updated) {
        return false;
    }
...
{
    std::optional<Permissions> updated = this->readDirectoryPermissions(path);
    if (!updated) {
        return false;
    }
    ...
    if (recursive) {
        ...
        success &= this->readDirectory(path, recursive, [this, &path, &operation, &permissions, &success](std::string const &name) {
            ...
            std::optional<Type> type = this->type(full);
            if (!type) {
                return false;
            }
...
{
#if _WIN32
    std::optional<Type> fromType = this->type(from);
    if (fromType != Type::File && fromType != Type::SymbolicLink) {
        return false;
    }

    std::optional<Type> toType = this->type(to);
...
std::optional<std::string> DefaultFilesystem::
...
{
    std::function<bool(std::string const &, std::optional<std::string> const &)> process =
        [this, &recursive, &cb, &process](std::string const &absolute, std::optional<std::string> const &relative) -> bool {
#if _WIN32
    ...
    return process(path, std::nullopt);
}
