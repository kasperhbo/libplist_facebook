...
#include "../Util/DefaultFilesystem.h"
...
static bool
Revert(std::unique_ptr<plist::Object> *root, Filesystem const *filesystem, std::string const &path, std::optional<plist::Format::Any> *format = nullptr)
{
    if (path.empty()) {
...
main(int argc, char **argv)
{
     ...
     */
    std::unique_ptr<plist::Object> root;
    std::optional<plist::Format::Any> saveFormat;
     ...
     */
    std::optional<plist::Format::Any> printFormat;
    if (options.xml()) {
        plist::Format::XML xml = plist::Format::XML::Create(plist::Format::Encoding::UTF8);
